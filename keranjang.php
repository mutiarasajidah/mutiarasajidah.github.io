...
<body>
    <div class="container">
        ...
        <form action="Checkout.php" method="post"> <!-- Form untuk pengiriman data -->
            <table>
                <thead>
                    <tr>
                        <th>Nama Produk</th>
                        ...
                        foreach ($_SESSION['cart'] as $id => $item) {
                            if (isset($item['name'], $item['price'], $item['quantity'])) {
                                ...
                                echo "<tr>
                                    ...
                                    <td>
                                        <input type='number' class='quantity-input' name='quantity[$id]' value='{$item['quantity']}' min='1'>
                                        <button type='submit' name='update' value='$id'>Update</button>
                                    </td>
                                    <td>Rp" . number_format($subtotal, 0, ',', '.') . "</td>
                                    <td><input type='checkbox' name='checkout[]' value='$id'></td>
                                    <td>
            ...
            function calculateTotalPrice() {
                ...
                checkboxes.forEach(checkbox => {
                    if (checkbox.checked) {
                        ...
                        const price = parseFloat(row.cells[1].innerText.replace('Rp', '').replace('.', '').replace(',', '.'));
